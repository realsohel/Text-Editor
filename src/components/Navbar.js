...
export default function Navbar(props) {
    ...
    return (
        <nav className={`navbar navbar-expand-lg bg-${props.mode}`}>
            ...
            <Link className={`navbar-brand text-${props.text}`} to="/">{props.title}</Link> {/* props */}
            ...
            <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                ...
                <Link className={`nav-link active text-${props.text}`} aria-current="page" to="/">{props.home}</Link>
                </li>
                <li className="nav-item">
